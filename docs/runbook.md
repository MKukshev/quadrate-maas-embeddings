...
- Ограничения rate limit. Конфигурация в `configs/rate_limits.yaml`; при получении 429 можно временно поднять `capacity`/`refill_rate` или добавить отдельные лимиты для ключей в `per_api_key`, затем перезапустить Router. Для анонимного режима используйте секцию `anonymous` (если `allow_anonymous_without_api_keys=true` в `configs/auth.yaml`) — ответы вернут 429 с сообщением `Rate limit exceeded, retry after ...`, а в логах появится запись о ключе `anonymous`.
